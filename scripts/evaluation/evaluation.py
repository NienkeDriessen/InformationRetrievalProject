...
def ndcg_score(ground_truth: pd.DataFrame, retrieved_list: pd.DataFrame, k: int) -> float:
    ...
    dcg = (np.array(retrieved_list[:k]['relevance_score_target']) / log).sum()  # TODO: check slice is correct + change to use right relevance scores
...
def evaluate_query(ground_truth: pd.DataFrame, retrieved_list: pd.DataFrame, ks: [int], metadata: pd.DataFrame):
    ...
    prediction = retrieved_list.copy().merge(ground_truth, on='image_path', how='left', suffixes=('', '_target'))
...
def evaluate_all_queries(queries: [str], ground_truth: dict, retrieved_list: dict, ks: [int], metadata: pd.DataFrame) \
    ...
    """
    # get all scores
    all_scores = [evaluate_query(ground_truth[q]['ranked_list'], retrieved_list[q]['ranked_list'], ks, metadata) for q in queries]
    # get average scores
    avg_scores = {}
