...
def filter_relevance_labels(corpus: str, ground_truth: pd.DataFrame, retrieved_list: pd.DataFrame,
                            metadata: pd.DataFrame) -> pd.DataFrame:
    """
    Set relevance labels for particular corpus, setting others to irrelevant (0).
    :param corpus: corpus to filter for (e.g., 'og', 'c1', 'c2', 'c3')
    :param ground_truth: ground truth of original images
    :param retrieved_list: retrieved list
    :param metadata: metadata for finding the original image path for altered images
    :return:
    """
...
def evaluate_query(ground_truth: pd.DataFrame, retrieved_list: pd.DataFrame, ks: [int], metadata: pd.DataFrame):
    ...
    ground_truth['relevance_score'] = ground_truth['ps_rel_feedback']  # TODO: change depending on which used
    ndcg_scores = {}
    for k in ks:
        ...
        prediction = filter_relevance_labels('bin1', ground_truth, retrieved_list, metadata)
        ndcg_c1 = ndcg_score(ground_truth, prediction, k)
        prediction = filter_relevance_labels('bin2', ground_truth, retrieved_list, metadata)
        ndcg_c2 = ndcg_score(ground_truth, prediction, k)
        prediction = filter_relevance_labels('bin3', ground_truth, retrieved_list, metadata)
        ndcg_c3 = ndcg_score(ground_truth, prediction, k)
        prediction = filter_relevance_labels('bin4', ground_truth, retrieved_list, metadata)
        ndcg_c4 = ndcg_score(ground_truth, prediction, k)
        prediction = filter_relevance_labels('bin5', ground_truth, retrieved_list, metadata)
        ndcg_c5 = ndcg_score(ground_truth, prediction, k)
        ndcg_scores[str(k)] = {
            'ndcg_all': ndcg_all,
            'ndcg_og': ndcg_og,
            'ndcg_c1': ndcg_c1,
            ...
            'relD_og_c4': relative_d_score(ndcg_og, ndcg_c4),
            'relD_og_c5': relative_d_score(ndcg_og, ndcg_c5),
            'relD_c1_c2': relative_d_score(ndcg_c1, ndcg_c2),
            'relD_c1_c3': relative_d_score(ndcg_c1, ndcg_c3),
            'relD_c1_c4': relative_d_score(ndcg_c1, ndcg_c4),
            'relD_c1_c5': relative_d_score(ndcg_c1, ndcg_c5),
            'relD_c2_c3': relative_d_score(ndcg_c2, ndcg_c3),
            'relD_c2_c4': relative_d_score(ndcg_c2, ndcg_c4),
            'relD_c2_c5': relative_d_score(ndcg_c2, ndcg_c5),
            'relD_c3_c4': relative_d_score(ndcg_c3, ndcg_c4),
            'relD_c3_c5': relative_d_score(ndcg_c3, ndcg_c5),
            'relD_c4_c5': relative_d_score(ndcg_c4, ndcg_c5),
        }
    return ndcg_scores
...
def evaluate_all_queries(queries: [str], ground_truth: dict, retrieved_list: dict, ks: [int],
    ...
    :param ks: values of k for nDCG@k
    :param metadata: Metadata dataframe for finding the original image path for altered images.
    :param save_folder: folder to save results to
    :return: dictionary for nDCG and relative delta scores for each corpus
    """
    # get all scores
    ...
    for k in ks:
        avg_scores[str(k)] = {
            ...
            'ndcg_c4': np.mean([s[str(k)]['ndcg_c4'] for s in all_scores]),
            'ndcg_c5': np.mean([s[str(k)]['ndcg_c5'] for s in all_scores]),
            ...
            'relD_og_c4': np.mean([s[str(k)]['relD_og_c4'] for s in all_scores]),
            'relD_og_c5': np.mean([s[str(k)]['relD_og_c5'] for s in all_scores]),
            'relD_c1_c2': np.mean([s[str(k)]['relD_c1_c2'] for s in all_scores]),
            'relD_c1_c3': np.mean([s[str(k)]['relD_c1_c3'] for s in all_scores]),
            'relD_c1_c4': np.mean([s[str(k)]['relD_c1_c4'] for s in all_scores]),
            'relD_c1_c5': np.mean([s[str(k)]['relD_c1_c5'] for s in all_scores]),
            'relD_c2_c3': np.mean([s[str(k)]['relD_c2_c3'] for s in all_scores]),
            'relD_c2_c4': np.mean([s[str(k)]['relD_c2_c4'] for s in all_scores]),
            'relD_c2_c5': np.mean([s[str(k)]['relD_c2_c5'] for s in all_scores]),
            'relD_c3_c4': np.mean([s[str(k)]['relD_c3_c4'] for s in all_scores]),
            'relD_c3_c5': np.mean([s[str(k)]['relD_c3_c5'] for s in all_scores]),
